...
| `sql/2026-08-21-subscription-tool-name.sql` | `subscriptions.tool_name`, backfilled from the company each plan used to hang off, and the NOT NULL comes off `subscriptions.vendor_id` | **before the code — and after `sql/2026-08-20-subscriptions.sql`, which creates the table. The register's projection now names `tool_name`, so without it every read of the tools screen 500s, not just the new field — and while `vendor_id` is still NOT NULL the form cannot save a subscription at all** |
| `sql/2026-08-19-tds-on-payroll.sql` | `payroll_lines.tds_basis` and `tds_declared_investment` — the tax becomes a calculation that carries the rule that produced it | **before the code — columns on an existing table, so without it every read of the salary sheet 500s** |
