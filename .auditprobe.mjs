...
page.on("response", async r => {
  if (r.url().includes("/api/") && r.status()>=400) {
    ...
    console.log("   " + r.status() + "  " + r.url().replace(/.*\/api/,"") + "\n      " + body);
  }
});
await page.goto("http://localhost:3000/settings",{waitUntil:"networkidle0",timeout:120000}).catch(()=>{});
await new Promise(r=>setTimeout(r,2200));
console.log("clicking 'What changed'...");
await page.evaluate(() => { [...document.querySelectorAll('[role="tab"]')].find(t=>t.textContent.trim()==="What changed")?.click(); });
await new Promise(r=>setTimeout(r,2600));
const msg = await page.evaluate(() => {
  const main=document.querySelector("main")||document.body;
  return main.innerText.split("\n").filter(l=>/couldn|fail|error|went wrong/i.test(l)).slice(0,3);
});
console.log("panel says: " + JSON.stringify(msg));
await browser.close();
